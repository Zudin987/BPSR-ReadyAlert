// Keep the existing assertion-guarded Rust generator intact. Generate a shaped
// version of the Pixel core because other legacy builders still consume its
// historical source; never mutate files checked in under src/ at build time.
...
fn main(){
    ...
    once(&mut src,"let rounded: i32 = 2; // DWMWCP_ROUND",
        "let rounded: i32 = 1; // DWMWCP_DONOTROUND: exact 4-DIP HWND region owns geometry, not this OS preference",
        "do not double-round with OS corner preference");
    src.push_str(r#"
...
/// Convert a 96-DPI token to physical pixels exactly once and then invert the
/// legacy meter's MM_ANISOTROPIC viewport. Text scale is not shape scale.
...
#[cfg(test)] mod two_shape_token_tests {
    ...
    #[test] fn controls_and_containers_are_the_only_tokens(){
        assert_eq!(SHAPE_RADIUS_CONTROL,2);assert_eq!(SHAPE_RADIUS_CONTAINER,4);
...
"#);
    ...
    let mut dialog=fs::read_to_string(base.join("src/ui_pixel_dialog.rs")).expect("pixel dialog");
    once(&mut dialog,"(8, 12, 16, 20)","(2, 4, 4, 4)","legacy dialog test matches new tokens");
    ...
    once(&mut dialog,"crate::ui::fit_window(hwnd, owner, true);",
        ...
        "modal window shell, not just client");

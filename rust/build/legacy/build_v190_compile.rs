...
fn replace_once(source:&mut String,from:&str,to:&str,label:&str){let count=source.matches(from).count();assert_eq!(count,1,"v1.9.0 patch `{label}` expected one match, found {count}");*source=source.replacen(from,to,1);}
fn replace_between(source:&mut String,start:&str,end:&str,replacement:&str,label:&str){let count=source.matches(start).count();assert_eq!(count,1,"v1.9.0 patch `{label}` start expected one match, found {count}");let begin=source.find(start).expect("start checked");let rel_end=source[begin..].find(end).unwrap_or_else(||panic!("v1.9.0 patch `{label}` end anchor missing"));source.replace_range(begin..begin+rel_end,replacement);}

fn main(){
previous::run();
let out=PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

// The v1.8 telemetry constructor owns the canonical row payload. Initialize the
// v1.9-only values there; telemetry_v190 then enriches them before forwarding.
let telemetry_path=out.join("telemetry_v170_fixed.rs");
let mut telemetry=fs::read_to_string(&telemetry_path).expect("read generated v1.8.5 telemetry");
replace_once(&mut telemetry,"                is_local: uid == self.local_uid && self.local_uid > 0,\n                attributes,","                is_local: uid == self.local_uid && self.local_uid > 0,\n                active_dps: 0.0,\n                active_hps: 0.0,\n                active_dtps: 0.0,\n                is_party: false,\n                attributes,","v1.9 row defaults");
fs::write(&telemetry_path,telemetry).expect("write v1.9 telemetry defaults");
...
replace_once(&mut overlay,"paths::AppPaths,\n};","paths::AppPaths,\nhistory::{self, HistoryEncounter},\n};","history imports");
replace_once(&mut overlay,"capture_status: String,\nhover_text: Option<String>,","capture_status: String,\nhistory: Vec<HistoryEncounter>,\nhistory_index: Option<usize>,\nhover_text: Option<String>,","history state fields");
replace_once(&mut overlay,"let state=Box::new(State{kind","let history_records=if kind==Kind::Dps{history::load_recent(&paths.root,snapshot.meter.history_limit)}else{Vec::new()};let state=Box::new(State{kind","load history at overlay creation");
replace_once(&mut overlay,"capture_status:\"Starting capture...\".into(),hover_text:None,hover_x:0,hover_y:0}","capture_status:\"Starting capture...\".into(),history:history_records,history_index:None,hover_text:None,hover_x:0,hover_y:0}","history state initialization");

replace_between(&mut overlay,"pub unsafe fn update_dps(hwnd:HWND,snapshot:DpsSnapshot){","pub unsafe fn update_mechanics",r#"pub unsafe fn update_dps(hwnd:HWND,snapshot:DpsSnapshot){with_state(hwnd,|state|{let rolled=history::encounter_rolled(&state.dps,&snapshot);if rolled{archive_live_snapshot(state);if !state.features.read().map(|f|f.meter.remember_scroll).unwrap_or(false){state.scroll=0;}}if state.history_index.is_none()&&!state.detail_hwnd.is_null()&&IsWindow(state.detail_hwnd)!=0&&state.detail_uid!=0{if let Some(row)=snapshot.rows.iter().find(|row|row.uid==state.detail_uid).cloned(){update_detail(state.detail_hwnd,row,snapshot.encounter_ms);}}state.dps=snapshot;let total=meter_rows(state).len();state.scroll=state.scroll.min(total.saturating_sub(1));});}
...
// Patch only the DPS control match. Do not replace the surrounding on_click
// region because later v1.8 builds intentionally place hover/resize helpers there.
replace_once(&mut overlay,"225..=298=>{crate::telemetry::request_manual_reset();state.dps=DpsSnapshot::default();state.scroll=0;},_=>{}","225..=298=>{if state.history_index.is_none(){archive_live_snapshot(state);crate::telemetry::request_manual_reset();state.dps=DpsSnapshot::default();state.scroll=0;}},304..=336=>history_older(state),341..=395=>{state.history_index=None;state.scroll=0;},400..=432=>history_newer(state),_=>{}","history control click arms");
...
replace_between(&mut overlay,"#[derive(Clone,Copy)]struct DpsRowLayout","unsafe fn draw_percent(",include_str!("overlay_v190_meter_patch.txt"),"history meter body");
replace_between(&mut overlay,"unsafe fn clamp_scroll(hwnd:HWND,state:&mut State){","unsafe fn open_detail",r#"unsafe fn clamp_scroll(hwnd:HWND,state:&mut State){let mut rc:RECT=std::mem::zeroed();GetClientRect(hwnd,&mut rc);let(total,visible)=if state.kind==Kind::Dps{(meter_rows(state).len(),visible_dps_rows(rc.bottom))}else{let tracked=state.features.read().map(|f|f.mechanic_attributes.tracked.len()).unwrap_or(0);let top=TOOLBAR_H+5+if tracked>0{MECH_ATTR_H}else{0}+MECH_CONSUMABLE_H;let visible=((rc.bottom-top)/MECH_ROW_H).max(0)as usize;let now=now_ms();let total=state.mechanics.rows.iter().filter(|row|row.persistent||row.expires_unix_ms<=0||row.expires_unix_ms>now).count();(total,visible)};state.scroll=state.scroll.min(total.saturating_sub(visible.max(1)));}
"#,"filtered scroll bounds");
replace_once(&mut overlay,"encounter_ms:state.dps.encounter_ms","encounter_ms:view_snapshot(state).encounter_ms","historical inspector duration");
replace_once(&mut overlay,"let(width,height)=if state.kind==Kind::Dps{(430,390)}else{(720,580)};","let(width,height)=if state.kind==Kind::Dps{(500,610)}else{(720,580)};","larger DPS settings window");
replace_between(&mut overlay,"unsafe fn paint_feature_settings(hwnd:HWND,state:&SettingsState){","fn mutate_features",include_str!("overlay_v190_settings_patch.txt"),"v1.9 settings body");
fs::write(&overlay_path,overlay).expect("write v1.9 generated overlay");
println!("cargo:rerun-if-changed=build_v190_compile.rs");println!("cargo:rerun-if-changed=overlay_v190_meter_patch.txt");println!("cargo:rerun-if-changed=overlay_v190_settings_patch.txt");println!("cargo:rerun-if-changed=src/history.rs");println!("cargo:rerun-if-changed=src/telemetry_v190.rs");
}

...
// A repeated call site must be replaced one occurrence at a time: the two raid
// columns deliberately share source text but have independent width decisions.
fn edit(src:&mut String,old:&str,new:&str,n:usize,id:&str){assert_eq!(src.matches(old).count(),n,"compact {id} expected {n} anchors");*src=src.replacen(old,new,1);}
...
inside(&mut src,"unsafe fn paint_compact_raid_rows(","fn raid_painted_player_column(","                show_total,","                left_total,",2,"left total first of two call sites");
inside(&mut src,"unsafe fn paint_compact_raid_rows(","fn raid_painted_player_column(","                show_total,","                right_total,",1,"right total remaining call site");

...
function apply(){const q=lower(search?.value.trim());const wantedChannel=channel?.value||'';const wantedSender=lower(sender?.value.trim());let shown=0;for(const msg of messages){const name=lower(msg.dataset.sender);const textEl=msg.querySelector('.message-text');const body=lower(textEl?.dataset.original);const visible=(activeDate==='all'||msg.dataset.date===activeDate)&&(!wantedChannel||msg.dataset.channel===wantedChannel)&&(!wantedSender||name.includes(wantedSender))&&(!q||(name+' '+body).includes(q));msg.hidden=!visible;if(visible){shown++;highlight(textEl,q);}else if(textEl&&q)highlight(textEl,'');}for(const day of days)day.hidden=!day.querySelector('.message:not([hidden])');if(result)result.textContent=shown.toLocaleString()+' message'+(shown===1?'':'s');if(empty)empty.hidden=shown!==0;title.textContent=activeDate==='all'?'All retained logs':activeDate;}
...
for(const button of document.querySelectorAll('button.sender'))button.addEventListener('click',()=>{sender.value=button.textContent||'';apply();search?.focus();});

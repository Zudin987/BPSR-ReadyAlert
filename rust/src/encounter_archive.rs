mod archive_impl {
    ...
    pub fn generate_inner(root: &std::path::Path) -> Result<std::path::PathBuf, String> { generate(root) }
}
...
use windows_sys::Win32::{Foundation::HWND, UI::{Shell::ShellExecuteW, WindowsAndMessaging::SW_SHOWNORMAL}};

fn replace_once(source: &mut String, from: &str, to: &str, label: &str) -> Result<(), String> {
    let count = source.matches(from).count();
    if count != 1 { return Err(format!("encounter-history archive UX {label} expected one anchor, found {count}")); }
...
fn upgrade_archive_ui(path: &Path) -> Result<(), String> {
    ...
    replace_once(&mut html,
        ...
        "Hide Misc control")?;
    replace_once(&mut html, "No saved encounter matches this search.", "No saved encounter matches the current filters.", "empty filter message")?;
    replace_once(&mut html, "</style>", r#"
...
function applySearch(){const q=(search.value||'').trim().toLocaleLowerCase(),hideMisc=$('#hide-misc')?.getAttribute('aria-pressed')==='true';filtered=DATA.filter(e=>{if(hideMisc&&!isMainContent(e))return false;if(!q)return true;const rows=e.snapshot?.rows||[];return [encounterTitle(e),e.target_name,e.context?.difficulty,e.context?.benchmark_name,...rows.map(r=>r.name),...rows.map(r=>r.subprofession_name)].join(' ').toLocaleLowerCase().includes(q)});compareIds=compareIds.filter(id=>filtered.some(e=>e.id===id));if(!filtered.some(e=>e.id===selectedId))selectedId=filtered[0]?.id??null;selectedPlayer=0;tab='overview';compareMode=false;renderList();renderDetail();}"#;
    replace_once(&mut html, old_apply_search, new_apply_search, "main-content filter")?;
...
setHideMisc(loadHideMisc());hideMiscBtn?.addEventListener('click',()=>{const next=hideMiscBtn.getAttribute('aria-pressed')!=='true';setHideMisc(next);saveHideMisc(next);applySearch()});$('#refresh').onclick=()=>location.reload();compareBtn.onclick=()=>{if(compareIds.length===2){compareMode=true;renderDetail()}};search.addEventListener('input',applySearch);document.addEventListener('keydown',ev=>{if(ev.key==='/'&&document.activeElement!==search){ev.preventDefault();search.focus()}else if(ev.key==='Escape'&&search.value){search.value='';applySearch();search.focus()}});applySearch();"#;
    ...
    fs::write(path, html.as_bytes()).map_err(|e| format!("write upgraded encounter-history archive UX: {e}"))
}

/// Preserve the last-good HTML if any generator/upgrade fails midway.
pub fn generate(root: &Path) -> Result<PathBuf, String> {
    ...
    if had_target { fs::copy(&target, &guard).map_err(|e| format!("preserve previous encounter history: {e}"))?; }
    else { let _ = fs::remove_file(&guard); }
    let generated = archive_impl::generate_inner(root).and_then(|path| {
    ...
    match generated {
        Ok(path) => { let _ = fs::remove_file(&guard); Ok(path) }
        Err(err) => {
            let restore = if had_target {
                fs::copy(&guard, &target).map(|_| ()).map_err(|e| format!("restore previous encounter history: {e}"))
            } else { Ok(()) };
            let _ = fs::remove_file(&guard);
            match restore { Ok(()) => Err(err), Err(restore_err) => Err(format!("{err}; {restore_err}")) }
        }
    }
}

/// Regenerate the local report only when the user opens it; never on the capture thread.
pub unsafe fn open_local(owner: HWND) {
    ...
    let path = match generate(&root) {
        ...
        Err(err) => {
            ...
            crate::win::message_box("BPSR ReadyAlert - Encounter History", &format!("Could not prepare Encounter History.\n\n{err}"), true);
            return;
        }
    };
    let operation: Vec<u16> = "open".encode_utf16().chain(std::iter::once(0)).collect();
    let file: Vec<u16> = path.as_os_str().encode_wide().chain(std::iter::once(0)).collect();
    let result = ShellExecuteW(owner, operation.as_ptr(), file.as_ptr(), null(), null(), SW_SHOWNORMAL);
    if result as isize <= 32 {
        crate::logging::write(format!("encounter-history: ShellExecuteW failed code={} path={}", result as isize, path.display()));
        crate::win::message_box("BPSR ReadyAlert - Encounter History", "Encounter History was generated, but Windows could not open it in your default browser.", true);
    }
}

...
use windows_sys::Win32::{
 ...
 Graphics::Gdi::{DrawTextW,GetDC,GetStockObject,ReleaseDC,SelectObject,SetBkMode,SetTextColor,HDC,DEFAULT_GUI_FONT,TRANSPARENT},
...
unsafe fn child(parent:HWND,class:&str,text:&str,id:i32,x:i32,y:i32,w:i32,h:i32,extra:u32)->HWND{let hwnd=CreateWindowExW(0,wide(class).as_ptr(),wide(text).as_ptr(),WS_CHILD|WS_VISIBLE|if id!=0{WS_TABSTOP}else{0}|extra,x,y,w,h,parent,id as usize as _,GetModuleHandleW(null()),null_mut());if class.eq_ignore_ascii_case("BUTTON"){crate::ui_modern::theme_control(hwnd);}else if class.eq_ignore_ascii_case("STATIC")&&!hwnd.is_null(){SendMessageW(hwnd,0x30,GetStockObject(DEFAULT_GUI_FONT)as usize,1);}hwnd}
unsafe fn body_height(owner:HWND,body:&str)->i32{let work=crate::ui::work_area(owner);let max=((work.bottom-work.top-36).clamp(215,620)-111).max(72);let dc=GetDC(null_mut());if dc.is_null(){return 104.min(max);}let old=SelectObject(dc,GetStockObject(DEFAULT_GUI_FONT));let w=wide(body);let mut r=RECT{left:0,top:0,right:402,bottom:0};DrawTextW(dc,w.as_ptr(),-1,&mut r,DT_CALCRECT|DT_WORDBREAK|DT_NOPREFIX);SelectObject(dc,old);ReleaseDC(null_mut(),dc);(r.bottom-r.top+8).clamp(72,max)}
unsafe fn paint_bg(hwnd:HWND,hdc:HDC,s:&State){let mut r:RECT=std::mem::zeroed();GetClientRect(hwnd,&mut r);crate::ui_modern::fill_mist_background(hdc,&r);crate::ui_modern::fill_round_rect(hdc,RECT{left:18,top:20,right:22,bottom:20+s.body_h},s.color,1);}
...
unsafe extern "system" fn proc(hwnd:HWND,msg:u32,wparam:WPARAM,lparam:LPARAM)->LRESULT{if msg==WM_NCCREATE{let cs=lparam as*const CREATESTRUCTW;if !cs.is_null(){SetWindowLongPtrW(hwnd,GWLP_USERDATA,(*cs).lpCreateParams as isize);}}let p=GetWindowLongPtrW(hwnd,GWLP_USERDATA)as*mut State;match msg{
 WM_CREATE=>{mist_titlebar(hwnd);if !p.is_null(){let s=&*p;child(hwnd,"STATIC",&s.body,0,34,20,402,s.body_h,0);let y=38+s.body_h;let bw=if s.buttons.len()>=3{96}else{112};let gap=10;let total=bw*s.buttons.len()as i32+gap*s.buttons.len().saturating_sub(1)as i32;let start=(466-total-18).max(24);let mut first=null_mut();for(i,b)in s.buttons.iter().enumerate(){let h=child(hwnd,"BUTTON",&b.label,b.id,start+i as i32*(bw+gap),y,bw,30,BS_OWNERDRAW);if first.is_null(){first=h;}}if !first.is_null(){crate::ui::SetFocus(first);}}0},
 WM_ERASEBKGND=>{if p.is_null(){0}else{paint_bg(hwnd,wparam as HDC,&*p);1}},
 WM_CTLCOLORSTATIC=>{let hdc=wparam as HDC;SetBkMode(hdc,TRANSPARENT as i32);SetTextColor(hdc,crate::ui_modern::BPSR_TEXT);crate::ui_modern::mist_bg_brush()as LRESULT},

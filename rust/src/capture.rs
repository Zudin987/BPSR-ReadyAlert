...
fn select_device(api: &PcapApi, settings: &AppSettings) -> Result<SelectedDevice, String> {
    ...
    if let Some(saved) = resonance_logs_device() {
        if let Some(d) = devices.iter().find(|d| d.name.eq_ignore_ascii_case(&saved)) {
            return Ok(selected(d, "Resonance Logs CN"));
        }
    }
...
fn resonance_logs_device() -> Option<String> {
    let mut roots = Vec::new();
    if let Some(v)=env::var_os("LOCALAPPDATA") { roots.push(std::path::PathBuf::from(v)); }
    if let Some(v)=env::var_os("APPDATA") { roots.push(std::path::PathBuf::from(v)); }
    for root in roots {
        for app in ["com.resonance-logs-cn","resonance-logs-cn","resonance-logs"] {
            for rel in [format!("{app}/stores/packetCapture.json"), format!("{app}/packetCapture.json")] {
                let path=root.join(rel);
                let Ok(text)=fs::read_to_string(path) else { continue; };
                let Ok(v)=serde_json::from_str::<Value>(&text) else { continue; };
                if v.get("method").and_then(Value::as_str).map(|x| x.eq_ignore_ascii_case("Npcap")).unwrap_or(false) {
                    if let Some(d)=v.get("npcapDevice").and_then(Value::as_str).map(str::trim).filter(|x| !x.is_empty()) { return Some(d.to_string()); }
                }
            }
        }
    }
...
struct CaptureProcessor {
    tx:Sender<AppEvent>, identity:Arc<RwLock<Option<PlayerIdentity>>>, chat:ChatRuntime,
...
impl CaptureProcessor {
    fn new(tx:Sender<AppEvent>, identity:Arc<RwLock<Option<PlayerIdentity>>>, chat:ChatRuntime)->Self {
        Self { tx,identity,chat,flows:HashMap::new(),last_ready:None,last_queue:None,last_invite:(0,None),last_request:(0,None),sequence:0,last_valid_frame:None,last_cleanup:Instant::now() }
    }
    ...
    fn process_notify(&mut self,payload:&[u8],compressed:bool){if payload.len()<16{return;}let service=be64(payload,0).unwrap_or(0);let method=be32(payload,12).unwrap_or(0);let raw=&payload[16..];let owned;let body=if compressed{match zstd::stream::decode_all(Cursor::new(raw)){Ok(v)=>{owned=v;&owned[..]},Err(err)=>{logging::write(format!("packet: zstd notify failed: {err}"));return;}}}else{raw};

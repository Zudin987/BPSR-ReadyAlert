...
"#,
    ...
    replace_between(
        ...
        r#"unsafe fn open_detail(parent:HWND,state:&mut State,row:DpsRow){if !state.detail_hwnd.is_null()&&IsWindow(state.detail_hwnd)!=0{DestroyWindow(state.detail_hwnd);}let instance=GetModuleHandleW(null());let class=wide(DETAIL_CLASS);let wc=WNDCLASSW{lpfnWndProc:Some(detail_wnd_proc),hInstance:instance,hCursor:LoadCursorW(null_mut(),IDC_ARROW),lpszClassName:class.as_ptr(),..std::mem::zeroed()};if RegisterClassW(&wc)==0&&GetLastError()!=1410{return;}let mut pr:RECT=std::mem::zeroed();GetWindowRect(parent,&mut pr);let ptr=Box::into_raw(Box::new(DetailState{row:row.clone(),scroll:0,font:create_overlay_font(true),mode:state.sort_mode,encounter_ms:state.dps.encounter_ms}));let title=wide("Entity Inspector");let hwnd=CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_TOPMOST,class.as_ptr(),title.as_ptr(),WS_POPUP|WS_THICKFRAME,pr.right+8,pr.top,900,600,parent,null_mut(),instance,ptr.cast::<c_void>());if hwnd.is_null(){let boxed=Box::from_raw(ptr);if !boxed.font.is_null(){DeleteObject(boxed.font);}drop(boxed);return;}state.detail_hwnd=hwnd;state.detail_uid=row.uid;ShowWindow(hwnd,SW_SHOW);SetForegroundWindow(hwnd);}
